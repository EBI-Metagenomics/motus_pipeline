...
def main(args):
    ...
    parser.add_option(
        "-i", "--input", dest="input", default=None, help="Input sequence file"
    )
    parser.add_option(
        "-o",
        "--output",
    ...
    summary_out_file = (
        os.path.join(opts.output_dir, opts.output) if opts.output else None
    )
    base_distrib_file = (
        os.path.join(opts.output_dir, opts.base_distrib + prefix)
        if opts.base_distrib
        else None
    )
    len_bin_file = (
        os.path.join(opts.output_dir, opts.len_bin + prefix) if opts.len_bin else None
    )
    gc_bin_file = (
        os.path.join(opts.output_dir, opts.gc_bin + prefix) if opts.gc_bin else None
    )
    ...
    try:
        for rec in seq_iter(in_hdl, opts.type):
            ...
            if (opts.type == "fasta") and (re.match("^\s", rec.description)):
                sys.stderr.write(
                    "[error] invalid fasta file, first character following '>' in "
                    + "header must be non-whitespace\n"
                )
                os._exit(1)
            if (not opts.ignore_comma) and ("," in head):
                sys.stderr.write(
                    "[error] invalid sequence file, header may not contain a comma (,)\n"
                )
                os._exit(1)
            ...
            if not opts.fast:
                if opts.type == "fastq":
                    for q in qual:
                        ...
                        if ascii_value < 33 or ascii_value > 126:
                            sys.stderr.write(
                                (
                                    "[error] quality value with ASCII value: %d in"
                                    " sequence: %s (sequence number %d) is not within ASCII range 33 to 126\n"
                                )
                ...
                for i, c in enumerate(seq):
                    if c not in IUPAC:
                        try:
                            ...
                            sys.stderr.write(
                                (
                                    "[error] character '%s' (position %d) "
                                    "in sequence: %s (sequence number %d) is not a valid IUPAC code\n"
                                )
                                % (c, i, head, seqnum)
                            )
                            os._exit(1)
                            ...
                            sys.stderr.write(
                                (
                                    "[error] non-ASCII character at position %d in"
                                    " sequence: %s (sequence number %d) is not a valid IUPAC code\n"
                                )
                                % (i, head, seqnum)
                            )
                            os._exit(1)
                    charnum += 1
                    # loop added by H. Denise to generate data for nucleotide histogram.
                    # Initialise dictionary when new pos is encountered
                    ...
                    if c in char:
                        ...
                        base_distribution[charnum][
                            c
                        ] += 1  # added by H. Denise to generate data for nucleotide histogram.
                        # Increment count at looked up position
                ...
                if (char["G"] + char["C"]) > 0:
                    gc_r = "%.1f" % (
                        1.0 * (char["A"] + char["T"]) / (char["G"] + char["C"])
                    )
                gc_perc[gc_p] += 1
                gc_ratio[gc_r] += 1
                if ambig > 0:
    ...
    if seqnum == 0:
        sys.stderr.write(
            "[error] invalid %s file, unable to find sequence records\n" % opts.type
        )
        os._exit(1)
    ...
    if not summary_out_file:
        ...
        out_hdl.write("\n".join(stat_text) + "\n")
        out_hdl.close()
    # added by H. Denise to generate data for nucleotide histogram.
    # Write to file base count as % of all bases at each position
    if base_distrib_file:
        ...
        for i in range(1, charnum):
            sum = (
                base_distribution[i]["N"]
                + base_distribution[i]["G"]
                + base_distribution[i]["C"]
                + base_distribution[i]["T"]
                + base_distribution[i]["A"]
            )
            out_bd.write(
                str(i)
                + "\t"
                + str(round(float(base_distribution[i]["N"] * 100) / sum, 2))
                + "\t"
                + str(round(float(base_distribution[i]["G"] * 100) / sum, 2))
                + "\t"
                + str(round(float(base_distribution[i]["C"] * 100) / sum, 2))
                + "\t"
                + str(round(float(base_distribution[i]["T"] * 100) / sum, 2))
                + "\t"
                + str(round(float(base_distribution[i]["A"] * 100) / sum, 2))
                + "\n"
            )
        out_bd.close()
